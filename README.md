...
OR if you already have the multimer parameters, move them into the params dir in geminal as ColabDesign looks for these. If you do not, find AP- he can share the parameters if you don't want to follow the instructions above in using wget and tar for parameters. 
...
Install PyRosetta (Firewall Bypass)
...
Ensure you are physically inside the germinal_repo directory, then run:
# Install core ML libraries
uv pip install iglm torchvision==0.21.* chai-lab==0.6.1 torch==2.6.* torchaudio==2.6.* torchtyping==0.1.5 torch_geometric==2.6.*

# Pin specific JAX and Hydra versions for compatibility
...
BioHPC AF3 Paths (This is established on the bash script germina_run.sh)
You will need to pass these specific BioHPC module paths to your Slurm script:
Image (.sif): /project/apps/singularity-images/alphafold3/3.0.0/alphafold3_container.sif
Database: /project/apps_database/alphafold3/database_full
Parameters: /project/apps_database/alphafold3/model_params (Contains af3.bin.zst)

# Run Germinal: 
sbatch germinal_run.sh (if you'd like, ask AP for .sh script)

5. Live Monitoring & Tracking (a few of AP's recommendations): 
ssh into your node (eg. NucleusC097)

Launch monitoring tools:
nvtop (requires installation): Live GPU VRAM and compute utilization. Expect bursts of 100% compute during ColabDesign hallucination and heavy VRAM usage during AF3 prediction.

btop (requires installation): Live CPU/Memory utilization. Expect heavy core usage during PyRosetta scoring and AF3 MSA generation.

built-in with BioHPC: watch -n 1 nvidia-smi (this allows you to watch the GPU usage in real time with bursts of GPU usage like nvtop)
...
# Watch for fully accepted PDB files that pass all AF3 filters
watch -n 10 "ls -la /project/SCCC/Siegwart_lab/shared/germinal/results/test_vhh_run/trajectories/structures" (you will have to change the "test_vhh_run" to the name that you called for in your configuration settings within the .sh) 
